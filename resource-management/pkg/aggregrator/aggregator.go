...
const (
	...
	httpPrefix         = "http://"
	defaultPullInterval = time.Duration(10 * time.Millisecond) // 10ms as default pull interval
)
	...
	for i := 0; i < numberOfURLs; i++ {
		go func(i int) {
			...
			for {
				...
				// Get how many milliseconds of latency
				completeDuration := endPullAndProcess.Sub(startPullAndProcess)

				// For performance increasing, change interval from 100ms to 10ms, even less than 10ms
